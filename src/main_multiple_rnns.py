...
if __name__ == "__main__":
    ...
    start_time = args.num_rnns + '_' + datetime.now().strftime('%Y_%m_%d_%H_%M%S')
    if not args.feature_file:
        for i in range(int(args.num_rnns)):
            rnns[i] = Emb_RNNLM(rnn_params).to(device)
        ...
        for i in range(int(args.num_rnns)):
            rnns[i] =  Feature_RNNLM(rnn_params, feature_table).to(device)
