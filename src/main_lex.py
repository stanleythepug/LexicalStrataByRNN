...
#from model_bigram import Emb_RNNLM, Feature_RNNLM
from model import LexicalEmbeddingRNN
#from model import Emb_RNNLM, Feature_RNNLM
#from data_process import get_corpus_data, process_data, process_features
from data_process_no_padding import get_corpus_data, process_data, process_features
#from data_process_with_lexical_indices import get_corpus_data, process_data, process_features
#from training_for_embedding_capture import train_lm
#from training_print_zyookyoo_only import train_lm
from lex_training import train_lm
#from training import train_lm
#from training_next_seg import train_lm

DEFAULT_FEATURES_FILE = None
DEFAULT_D_EMB = 512
DEFAULT_LEXICAL_EMBEDDING_DIM = 512
DEFAULT_D_HID = 512
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--d_lex_emb', type=int, help='Number of dimensions for lexical embeddings.',
        default=DEFAULT_LEXICAL_EMBEDDING_DIM
    )
    parser.add_argument(
    ...
    inventory, phone2ix, ix2phone, training, dev, lex_training, lex_dev, num_words = process_data(
        raw_data, args.d_lex_emb, device, use_saved, dev=args.dev, training_split=args.training_split
    )
    ...
    RNN1 = LexicalEmbeddingRNN(rnn_params).to(device)
    if two_rnns:
        RNN2 = LexicalEmbeddingRNN(rnn_params).to(device)
    else:
        RNN2 = None
    print('Fitting lexical embedding model...')
    ...
    #sys.exit()
    train_lm(training, dev, lex_training, lex_dev, rnn_params, RNN1, RNN2, ix2phone, phone2ix, start_time)
    RNN1.eval()
    if two_rnns:
        RNN2.eval()
